...
import ImageChat from "./ai/aiimagechat";
...
const ScreenshotManager: React.FC<ScreenshotManagerProps> = ({
    ...
    const subscription = supabase
      ...
      .on(
        ...
        (payload) => {
          if (
            payload.new.team_id &&
            payload.new.walletAddress !== walletAddress
          ) {
  ...
  const fetchScreenshots = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .or(
          `walletAddress.eq.${walletAddress},team_id.in.(${teamIds.join(",")})`
        );
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      chrome.tabs.captureVisibleTab(
        ...
        async (dataUrl) => {
          ...
          // Reset the showExtractedText state
        }
      );
    });
  };
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      chrome.tabs.sendMessage(
        ...
        (response) => {
          ...
          if (chrome.runtime.lastError) {
            setError(
              `Failed to capture full page: ${chrome.runtime.lastError.message}`
            );
  ...
  const uploadScreenshot = async () => {
    ...
    try {
      ...
      const file = new File([blob], `${websiteName}-${Date.now()}.png`, {
        type: "image/png",
      });
  ...
  const downloadScreenshot = async (screenshotInfo: ScreenshotInfo) => {
    try {
      ...
      setError(
        `Failed to download ${screenshotInfo.fileName}. Please try again.`
      );
      console.error(err);
    }
  };
  ...
  const deleteScreenshot = async (id: number) => {
    try {
      ...
      setUploadedScreenshots(
        uploadedScreenshots.filter((screenshot) => screenshot.id !== id)
      );
    } catch (err: any) {
        ...
        <main className="flex-grow overflow-y-auto p-4 custom-scrollbar">
          <AnimatePresence mode="wait">
            ...
            /> */}
                      ...
                      <div className="fixed bottom-0 left-1/2 transform -translate-x-1/2 px-2 w-full max-w-xs flex justify-center">
                        <ImageChat imageData={latestScreenshot} />
                      </div>
                    </div>
                  </div>
                  {/* ImageChat moved below the button grid */}
                </div>
              </motion.div>
            )}
...
const TabButton: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`p-2 rounded-full flex items-center space-x-2 transition-colors duration-300 ${
      isActive
        ? "bg-primary text-background"
        : "bg-transparent text-text hover:bg-primary/20"
    }`}
    title={label}
  >
    {icon}
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-24 h-24 bg-surface rounded-full flex items-center justify-center group-hover:animate-capture-click">
            <FiCamera
              size={32}
              className="text-primary group-hover:text-accent-teal transition-colors duration-300"
            />
          </div>
        </div>
        <div className="absolute inset-0 flex items-center justify-center">
  ...
  <button
    ...
    className={`flex items-center justify-center ${
      disabled
        ...
        : "bg-gradient-to-r from-accent-orange to-accent-yellow hover:from-accent-orange/80 hover:to-accent-yellow/80"
...
}> = ({ screenshots, onPreview, onDownload, onDelete }) => {
    ...
    >
      {Object.keys(groupedScreenshots).length === 0 ? (
        ...
        Object.entries(groupedScreenshots).map(
          ([websiteName, websiteScreenshots]) => (
            <WebsiteGroup
              key={websiteName}
...
const ScreenshotItem: React.FC<{
  screenshot: ScreenshotInfo;
  onPreview: (screenshot: ScreenshotInfo) => void;
  onDownload: (screenshot: ScreenshotInfo) => void;
  onDelete: (id: number) => void;
}> = ({ screenshot, onPreview, onDownload, onDelete }) => (
  <div className="bg-gradient-to-br from-surface to-background rounded-lg p-2 flex items-center">
    <div
      className="flex-grow mr-2 truncate cursor-pointer"
    ...
    >
      ...
      <div className="flex items-center">
        <p className="text-xs text-text-secondary">
          ID: {screenshot.blobId.slice(0, 10)}...
        </p>
        {screenshot.team_id && (
    ...
    <div className="flex space-x-1">
      ...
      <ActionButton
        icon={<FiLink size={16} />}
        onClick={() => window.open(screenshot.suiUrl, "_blank")}
        title="View on Sui Explorer"
      />
      <ActionButton
        icon={<FiTrash2 size={16} />}
        onClick={() => screenshot.id && onDelete(screenshot.id)}
        title="Delete"
    ...
    <div className="fixed inset-0 bg-background bg-opacity-75 flex items-center justify-center z-50 ">
      <div className="bg-gradient-to-br from-surface to-background rounded-lg max-w-[90%] max-h-[90%] overflow-hidden shadow-lg">
        <div className="p-2 flex justify-between items-center bg-gradient-to-r from-primary/20 to-secondary/20 h-20">
          ...
          <button
            onClick={onClose}
            className="text-text hover:text-primary p-5  rounded transition-colors duration-300"
          >
            <FiX size={20} />
          </button>
        </div>
